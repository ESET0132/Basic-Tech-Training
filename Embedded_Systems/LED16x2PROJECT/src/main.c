...
void lcd_init(){
    ...
    _delay_ms(20); // Wait for clear display command to complete
...
void lcd_pulse(){
    ...
    _delay_ms(200); // Wait for command to process
...
int main(void){
    ...
    lcd_print("Stop Embedded C!"); // Print message
 
    lcd_command(0xC0); // Move cursor to second line, first position
    lcd_print("No Work"); // Print message
